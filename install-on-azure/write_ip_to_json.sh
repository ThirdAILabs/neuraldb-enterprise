...
PUBLIC_HEAD_IP=$(az vm list-ip-addresses --resource-group $resource_group_name --query "[?virtualMachine.name=='$vm_name'].virtualMachine.network.publicIpAddresses[0].ipAddress" -o tsv)
...
PRIVATE_HEAD_IP=$(az vm list-ip-addresses --resource-group $resource_group_name --query "[?virtualMachine.name=='$vm_name'].virtualMachine.network.privateIpAddresses[0]" -o tsv)
