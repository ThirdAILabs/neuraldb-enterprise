...
python3 ../nomad/nomad_jobs/submit_nomad_job.py --nomad-ip $PUBLIC_PROXY_SERVER_IP --hcl-template-filename ../nomad/nomad_jobs/model_bazaar_job.hcl.j2 DB_PASSWORD=$DB_PASSWORD SHARE_DIR=$SHARE_DIR PRIVATE_PROXY_SERVER_IP=$PRIVATE_PROXY_SERVER_IP PUBLIC_PROXY_SERVER_IP=$PUBLIC_PROXY_SERVER_IP PRIVATE_SERVER_IP=$PRIVATE_SERVER_IP JWT_SECRET=$jwt_secret ADMIN_USERNAME=$admin_name ADMIN_MAIL=$admin_mail ADMIN_PASSWORD=$admin_password