...
python3 ../nomad/nomad_jobs/submit_nomad_job.py --nomad-ip $PUBLIC_PROXY_SERVER_IP --hcl-template-filename ../nomad/nomad_jobs/model_bazaar_job.hcl.j2 DB_PASSWORD=$DB_PASSWORD SHARE_DIR=$SHARE_DIR PRIVATE_PROXY_SERVER_IP=$PRIVATE_PROXY_SERVER_IP PUBLIC_PROXY_SERVER_IP=$PUBLIC_PROXY_SERVER_IP PRIVATE_SERVER_IP=$PRIVATE_SERVER_IP JWT_SECRET=$jwt_secret GENAI_KEY=$genai_key