...
class AWSInfrastructure:
    def __init__(self, config, logger):
        ...
        self.config = config
        self.ec2 = boto3.client('ec2', region_name=self.config['network']['region'])
    ...
    def create_vpc(self):
        response = self.ec2.create_vpc(
            ...
            TagSpecifications=[{
                'ResourceType': 'vpc',
                'Tags': [{'Key': 'Project', 'Value': self.config['project']['name']}]
            }]
        )
    ...
    def create_route_table(self, vpc_id, igw_id, subnet_id):
        ...
        self.ec2.create_route(RouteTableId=rtb_id, DestinationCidrBlock='0.0.0.0/0', GatewayId=igw_id)
    ...
    def setup_security_group_rules(self, sg_id):
        rules = [
            ...
            {'IpProtocol': 'tcp', 'FromPort': 4646, 'ToPort': 4646, 'IpRanges': [{'CidrIp': '0.0.0.0/0'}]},
    ...
    def create_cluster_config(self, instances):
        ...
        web_ingress_public_ip = self.ec2.describe_instances(InstanceIds=[head_node_id])['Reservations'][0]['Instances'][0]['PublicIpAddress']
        web_ingress_private_ip = self.ec2.describe_instances(InstanceIds=[head_node_id])['Reservations'][0]['Instances'][0]['PrivateIpAddress']
        ...
        for client_node_id in client_node_ids:
            client_node_private_ip = self.ec2.describe_instances(InstanceIds=[client_node_id])['Reservations'][0]['Instances'][0]['PrivateIpAddress']
            new_node = {"private_ip": client_node_private_ip}
            config_data['nodes'].append(new_node)
        
        return config_data

    def cleanup_resources(self, vpc_id=None, sg_id=None, igw_id=None, subnet_ids=None, key_pair_name=None):
        try:
            ...
            if igw_id and vpc_id:
                try:
                    ...
                    self.logger.error(f"Failed to detach or delete Internet Gateway {igw_id}: {str(e)}")
            ...
            if subnet_ids:
                for subnet_id in subnet_ids:
                    try:
                        ...
                        self.logger.error(f"Failed to delete Subnet {subnet_id}: {str(e)}")
            ...
            if sg_id:
                try:
                    ...
                    self.logger.error(f"Failed to delete Security Group {sg_id}: {str(e)}")
            ...
            if key_pair_name:
                try:
                    ...
                    self.logger.error(f"Failed to delete Key Pair {key_pair_name}: {str(e)}")
